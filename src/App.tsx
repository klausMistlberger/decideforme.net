...
const App = () => {
  ...
  const getQueryParams = useCallback(() => {
    ...
    if (values.length >= 2) {
      const queryOptions = values.map(() => createOption());
  ...
  const optionChangeHandler = (event) => {
    const elementToUpdate = options.find((option) => option.id === event.target.id);
    elementToUpdate.value = event.target.value;
    const changeIndex = options.findIndex((element) => element === elementToUpdate);
  ...
  const createQueryUrl = () => {
    const queryArray = options.map((option, index) => `${index}=${option.value}`);
  ...
  return (
    <div className='App'>
      ...
      <form className='form'>
        ...
        <button type='button' className='result-button shadow' onClick={clickGoHandler}>
          GO
        </button>
      </form>
      ...
      <div className='utility-button-container'>
        <button type='button' className='utility-button reset-button shadow' onClick={clickResetHandler}>
          Reset
        </button>
        <button type='button' className='utility-button share-button shadow' onClick={clickShareHandler}>
          Share
        </button>
      </div>
    </div>
